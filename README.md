...
To generate Flowtype definitions you must first configure `protoc` to use this plugin and then specify where you want the TypeScript definitions to be written to using the `--flow_out` flag.
...
In the above example, the `generated` folder will contain both `.js` and `.flow.js` files which you can reference in your TypeScript project to get full type completion and make use of ES6-style import statements, eg:
...
The `generated` folder will now contain both `pb_service.js` and `pb_service.flow.js` files which you can reference in your TypeScript project to make RPCs.
